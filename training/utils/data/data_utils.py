...
def get_raw_dataset(dataset_name, output_path, seed, local_rank):
    ...
    elif "local/sft_jsonfile" in dataset_name:
        chat_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), os.path.pardir,
                         os.path.pardir, os.path.pardir))
        if not (os.path.isfile(chat_path + '/data/sft/train.json')
                and os.path.isfile(chat_path + '/data/sft/eval.json')):
            raise RuntimeError(
                f"Please check both the train.json and eval.json files in your applications/DeepSpeed-Chat/data/sft directory."
            )
        return raw_datasets.LocalJsonFileSplitDataset(output_path, seed, local_rank,
                                                 dataset_name, chat_path + '/data/sft')
    elif "local/rm_jsonfile" in dataset_name:
        chat_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), os.path.pardir,
                         os.path.pardir, os.path.pardir))
        if not (os.path.isfile(chat_path + '/data/rm/train.json')
                and os.path.isfile(chat_path + '/data/rm/eval.json')):
            raise RuntimeError(
                f"Please check both the train.json and eval.json files in your applications/DeepSpeed-Chat/data/rm directory."
            )
        return raw_datasets.LocalJsonFileSplitDataset(output_path, seed, local_rank,
                                                 dataset_name, chat_path + '/data/rm')
    elif "local/ppo_jsonfile" in dataset_name:
        chat_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), os.path.pardir,
                         os.path.pardir, os.path.pardir))
        if not (os.path.isfile(chat_path + '/data/ppo/train.json')
                and os.path.isfile(chat_path + '/data/ppo/eval.json')):
            raise RuntimeError(
                f"Please check both the train.json and eval.json files in your applications/DeepSpeed-Chat/data/ppo directory."
            )
        return raw_datasets.LocalJsonFileSplitDataset(output_path, seed, local_rank,
                                                 dataset_name, chat_path + '/data/ppo')
    else:
        raise RuntimeError(
...
class PromptDataset(Dataset):
    ...
    def __init__(self, prompt_dataset, chosen_dataset, reject_dataset,
                 pad_token_id, train_phase, label_dataset = None) -> None:
        super().__init__()
...
def create_dataset_split(current_dataset, raw_dataset, train_phase, tokenizer,
                         end_of_conversation_token, max_seq_len, human_text = None, assistant_text = None):
    prompt_dataset = []
    chosen_dataset = []
    reject_dataset = []
    label_dataset = []
                ...
                def find_all(text,substring):
                    ...
                    while index != -1:
                        ...
                        if index != -1:
                            ...
                            index += 1
                    return positions
                human_pos = find_all(chosen_sentence, human_text)
                assistant_pos =  find_all(chosen_sentence, assistant_text)
                label_token = chosen_token["input_ids"].clone()
                if len(human_pos) != len(assistant_pos):
                    continue
                for human, ass in zip(human_pos, assistant_pos):
                    # find the position of human/assistant_text after tokenize
                    start_ids = (chosen_token['offset_mapping'][:,:,0].squeeze(0) >= human).nonzero()
                    end_ids = (chosen_token['offset_mapping'][:,:,0].squeeze(0) >= (ass+len(assistant_text))).nonzero()

                    start = start_ids[0].item() if len(start_ids) > 0 else len(label_token)
                    end = end_ids[0].item() if len(end_ids) > 0 else len(label_token)

                    label_token[start:end] = -100
                chosen_dataset.append(chosen_token)
                label_dataset.append(label_token)
    ...
    return PromptDataset(prompt_dataset, chosen_dataset, reject_dataset,
                         tokenizer.pad_token_id, train_phase, label_dataset = label_dataset)


def create_dataset(local_rank, dataset_name, data_split, output_path,
                   train_phase, seed, tokenizer, end_of_conversation_token,
                   max_seq_len, human_text = None, assistant_text = None):
    ...
    train_dataset = create_dataset_split(train_dataset, raw_dataset,
                                         ...
                                         max_seq_len, human_text, assistant_text)
...
def get_unsupervised_data(args, tokenizer):
    if args.unsupervised_dataset_name == "pleisto/wikipedia-cn-20230720-filtered":
        unsupervised_raw_datasets = load_dataset("pleisto/wikipedia-cn-20230720-filtered")

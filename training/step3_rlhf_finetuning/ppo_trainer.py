...
class DeepSpeedPPOTrainer():
    ...
    def _generate_sequence(self, prompts, mask, step):
        ...
        if not hasattr(self.actor_model,"model"):
            kwargs = dict()
        elif self.actor_model.model.config.model_type == "llama":
        ...
        with torch.no_grad():
            seq = self.actor_model.module.generate(
                ...
                min_length=min(prompts.shape[1]+2, max_min_length),
    ...
    def generate_experience(self, prompts, mask, step):
        ...
        with torch.no_grad():
            ...
            # left padding to right padding, prompt_length is useless here.
            roll_idx = []
            roll_seq = []
            bs = seq.size(0)
            # roll input_id
            for i in range(bs):
                idx = ((seq[i] != pad_token_id).nonzero())[0].item()
                roll_idx.append(idx)
                roll_seq.append(torch.roll(seq[i], -idx))
            roll_seq = torch.stack(roll_seq)
            reward_score = self.reward_model.forward_value(
                input_ids = roll_seq,
                prompt_length=self.prompt_length)['chosen_end_scores'].detach()
            values = self.critic_model.forward_value(
                input_ids = roll_seq, return_value_only=True).detach()
            
            # roll value
            for i in range(bs):
                idx = roll_idx[i]
                values[i] = torch.roll(values[i], idx)
            
            values = values[:, :-1]
            
            if self.args.print_answers:
                print(
                    f"--- answers_reward --> step={step}, rank={torch.distributed.get_rank()}, {reward_score.tolist()}"
                )
    ...
    def train_rlhf(self, inputs):
        ...
        # roll
        for i in range(bs):
            idx = ((seq[i] != self.tokenizer.pad_token_id).nonzero())[0].item()
            roll_idx.append(idx)
            roll_seq.append(torch.roll(seq[i], -idx))
        roll_seq = torch.stack(roll_seq)
        value = self.critic_model.forward_value(input_ids = roll_seq,
                                                return_value_only=True,
                                                use_cache=False)
        # roll back
        for i in range(bs):
            idx = roll_idx[i]
            value[i] = torch.roll(value[i], idx)
        value = value[:, :-1]
